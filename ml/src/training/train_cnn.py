"""Train the simple CNN baseline on the generated PlantVillage splits.

This script trains the smallest comparison model in the project. It shares the
same split artifacts, class weights, and early-stopping logic as the other
training entrypoints so results remain comparable.
"""
...
def main() -> None:
    """Configure data, model, and optimizer then train the CNN baseline.

    Side Effects:
        Saves the best model weights and a training-history JSON file.
    """
    parser = argparse.ArgumentParser(
        description="Train SimpleCNN baseline on PlantVillage split CSVs."
    )
    ...
    parser.add_argument("--out-weights", default="ml/weights/cnn_best.pt", help="Output path for best checkpoint weights.")
    parser.add_argument("--out-history", default="ml/weights/cnn_history.json", help="Output JSON path for training history.")
